...
export default function JourneyMap() {
    ...
    const finishAll = async () => {
        ...
        if (session?.user?.email) {
            await supabase.from("user_progress").insert({ user_email: session.user.email.toLowerCase(), mission_id: SYSTEM_IDS.JOURNEY_MAP, score: total * 10, choice_label: "JOURNEY_MAP" });
        }
    };
